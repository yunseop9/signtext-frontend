...
function getUploadLabel(status) {
  if (status === ANALYSIS_STATUS.LOADING) return "분석 중";
  return "시작";
}

export function StartIndicatorButton({ inputMode, status, onUploadStart, disabled }) {
  ...
  return (
    <button
      ...
      onClick={isWebcam ? undefined : onUploadStart}
      aria-live="polite"
    >
      {label}
    </button>
